...
private List<Contact> contacts;
	...
	public void addContact (Contact contact) throws InvalidFormatException {
		...
		if (contact.getName().equals("") || contact.getTelefon().equals("") || contact.getAddress().equals(""))
			throw new InvalidFormatException("Empty field");
		else {
			if (contact.getTelefon().length() == 10){
				try{
					int i = 0;
					while (i < contact.getTelefon().length())
					{
						int cifra = Integer.parseInt(String.valueOf(contact.getTelefon().charAt(i)));
						i++;
					}
